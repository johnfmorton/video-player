...
export class VideoPlayer extends HTMLElement {
  ...
  private _render() {
    ...
    const sources = this.hasAttribute('sources') ? this.getAttribute('sources')! : undefined;
    ...
    const paddingTop = aspectRatios[aspectRatio as keyof typeof aspectRatios] || 56.25;
  ...
  private _loadVideo({
    ...
    // Self-hosted
      ...
      if (sources) {
        try {
          const parsed: { src: string; type: string }[] = JSON.parse(sources);
          sourcesHTML = parsed
  ...
  private _setupVimeoPlayer() {
    const iframe = this.shadowRoot!.querySelector('#vimeoPlayer') as HTMLIFrameElement;
    if (!iframe) return;
